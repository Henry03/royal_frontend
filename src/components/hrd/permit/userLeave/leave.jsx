...
const Leave = () => {
    ...
    const [filter, setFilter] = useState('request_date')
    ...
    const fetchData = (e) => {
        ...
        axios.post('/leavepermit/employee?page='+currentPage, input,
        {
            headers: {
        ...
        <>
            <div className="m-5 p-5 md:px-10 bg-base-100 rounded-xl">
                <div className='grid grid-flow-col items-center justify-between'>   
                    <h1 className="text-2xl font-semibold">Leave Permit</h1>
                    <button className="btn btn-primary w-40" onClick={()=>{document.getElementById('addLeave').showModal(),setLoad(false)}}>Add Leave</button>
                </div>
            </div>
                        ...
                        <tbody> 
                            ...
                            }
                            {
                                data.map((permit, index) => {
                                    return (
                                        <tr key={index}>
                                            <th>{new Date(permit.request_date).toLocaleString('en-SG', formattedDate)}</th>
                                            
                                            <Status track={permit.track} status={permit.status}/>
                                            <td className='btn btn-ghost' onClick={()=>{document.getElementById('leaveDetail').showModal();setId(permit.id);setIds(permit.id_staff)}}><SlOptions/></td>
                                        </tr>
                                    )
                                })
                            }
            ...
            <LeaveDetail id={id} ids={ids} fetchDataParent={fetchData}/>
            <AddLeave fetchDataParent={fetchData} value={load} setValue={setLoad}/>
